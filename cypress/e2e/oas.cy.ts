...
function checkOASResults(handler, response, item, language) {
    ...
    item.oasDetail[language].forEach((index) => {
        cy.getByData('oas').find('[data-cy="benefit-detail"]').should('exist').contains(runTextReplacements(handler,response,index))
      });


    //Detail Estimate
    if(item.oasDetail.estimate){
        cy.getByData('oas').find('[data-cy="benefit-estimate"]').should('exist').should('not.include.text', 'null')
    }
    ...
    if(item.collapsedText){
      ...
      item.collapsedText[language].forEach((index, step) => {
        cy.getByData('oas').find(`[data-cy="collapse-oas-${step}"]`).should('exist').contains(removeHtmlContent(index.object.heading)).click()
        if(index.estimate){
          //Estimate exists and is non-null
          cy.getByData('oas').find(`[data-cy="benefit-estimate"]`).should('exist').should('not.include.text', 'null')
        }else{
          cy.getByData('oas').find(`[data-cy="collapse-oas-${step}"]`).should('exist').contains(removeHtmlContent(index.object.text))
        }
      });
    }
    ...
    if(item.oasDeferral){
      //Detail
      cy.getByData('oas').find('[data-cy="benefit-detail"]').should('exist').contains(lang.detail.yourDeferralOptions)
      ...
      if(item.oasDeferral.table){
        //Table Heading Check
        cy.getByData('oas').find('[data-cy="deferral-title"]').should('exist').contains(lang.oasDeferralTable.title)

        cy.getByData('oas').find('[data-cy="deferral-heading"]').should('exist').contains(lang.oasDeferralTable.headingAmount)
        if(item.oasEligible[language] == "Eligible" || item.oasEligible[language] == "Admissible"){
          cy.getByData('oas').find('[data-cy="deferral-heading"]').should('exist').contains(lang.oasDeferralTable.headingAge)
        }else{
          cy.getByData('oas').find('[data-cy="deferral-heading"]').should('exist').contains(lang.oasDeferralTable.futureHeadingAge)
        }

        //Deferral Based On Age
        console.log("THISONE")
        console.dir(response)
        const deferralTableResults = item.ageYears < 65 ? oasFuture[0][65].oas.cardDetail.meta.tableData : oasFuture[0][item.ageYears+1].oas.cardDetail.meta.tableData

        deferralTableResults.forEach((index) => {
          cy.getByData('oas').find('[data-cy="deferral-table"]').should('exist').contains(index.age)
          cy.getByData('oas').find('[data-cy="deferral-table"]').should('exist').contains(removeHtmlContent(numberToStringCurrency(index.amount, language)))
        })
      }
    ...
    if (item.nextSteps) {
      ...
      item.nextSteps[language].forEach((index) => {
        cy.getByData('oas').find('[data-cy="next-step-content"]').should('exist').contains(runTextReplacements(handler, response, index))
      });
      //Check Income Limit Display
      if(item.nextSteps.limit){
        cy.getByData('oas').find('[data-cy="next-step-limit"]').should('exist').should('not.include.text', 'null')
      }
    }
    
    //Links
    const oasLinks = item.ageYears < 65 ? oasFuture[0][65].oas.cardDetail.links : oas.cardDetail.links;
