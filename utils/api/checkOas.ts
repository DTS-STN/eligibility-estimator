...
export default function checkOas(params: CalculationInput): BenefitResult {
  ...
  const meetsReqLegal =
    value.legalStatus === LegalStatusOptions.CANADIAN_CITIZEN ||
    value.legalStatus === LegalStatusOptions.PERMANENT_RESIDENT ||
    value.legalStatus === LegalStatusOptions.INDIAN_STATUS
  ...
  if (meetsReqIncome && meetsReqLegal && meetsReqYears) {
    ...
    if (value.legalStatus === LegalStatusOptions.SPONSORED) {
      return {
        eligibilityResult: ResultOptions.CONDITIONAL,
        entitlementResult: 0,
